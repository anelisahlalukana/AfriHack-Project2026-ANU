...
5. Create advisor/admin accounts through Supabase Authentication → Users, the server-side Admin API, or `npm run create-admin --prefix server` (bootstraps the first admin). Set their **app metadata** role to `advisor` or `admin` using the Admin API or the SQL example below. Do not use editable user metadata for roles. Existing client records still require an `advisor_id` matching their assigned staff user's Auth ID. Insurer logins for the provider portal are created by an admin at `/admin/users` (role **Provider**, then choose the insurer), or with `node scripts/create-provider-login.js "email" "Full Name" "Old Mutual"` from `server/`. See `docs/claims_flow.md`.
...
- There is no public sign-up. An advisor adds a client with **Add client** on the Clients page (`/clients`), which opens a popup; the server creates the client's login and emails an invitation to `/complete-registration` (set `CLIENT_APP_URL` in `server/.env`; defaults to `http://localhost:5173`). The client enters their ID number and a password, then the 6-digit code emailed to them, and lands on `/account`. The client area is mobile-first with a bottom navigation bar: Home (`/account`), Documents (`/account/documents`), Claims (`/account/claims`), Reminders (`/account/reminders`) and Profile (`/account/profile`). **Claims** opens "My claims and requests"; the **Log a claim** dropdown in its top-right corner starts a claim of the chosen type. **Ask for something** is the round chat button in the bottom-right corner of every client page: it asks the questions for a request (change of address, banking details, a border letter, ...) one at a time, shows a summary, and sends it. It is driven by the request types in the database (`request_types`), so new types appear in the chat without code changes. `/login` has one Username box: advisors and admins enter their email address, clients enter their 13-digit ID number (the one they registered with). Clients can't sign in with an email address. Insurers sign in with their email address and land on `/provider`.
