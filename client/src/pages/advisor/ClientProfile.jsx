...
export default function ClientProfile() {
  ...
  if (error) return <div className="card" role="alert"><p className="error">Unable to load this client. {error}</p><button onClick={retry}>Try again</button><Link to="/clients">Back to clients</Link></div>
  const { assets, liabilities, netWorth } = totals(client.client_financial_items)
  const maximum = Math.max(assets, liabilities, 1)
  return <><Link className="back" to={location.state?.from || '/clients'}><ArrowLeft size={16} /> All clients</Link><header className="page-heading"><div><h1>{client.first_name} {client.second_name} {client.surname}</h1><p>{client.contact_email || 'No email provided'} · <span className="badge">{client.status}</span></p></div><Link className="button primary" to={`/clients/${id}/edit`}><Pencil size={16} /> Edit financial needs analysis</Link></header>
    <div className="two-columns"><section className="card"><h2>Financial position</h2><p>Assets and liabilities, side by side.</p><div className="detail-row"><span>Net worth<small>Total assets less total liabilities</small></span><b>{money(netWorth)}</b></div>{[['Assets', assets], ['Liabilities', liabilities]].map(([label, value]) => <div key={label} className="chart-row"><div><span>{label}</span><b>{money(value)}</b></div><div className="track"><span style={{ width: `${value / maximum * 100}%` }} className={label === 'Assets' ? 'asset-bar' : ''} /></div></div>)}
