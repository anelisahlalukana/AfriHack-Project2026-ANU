const { CLIENT_ROLE_ID, ADVISOR_ROLE } = require("../constants/dev4");

function createDev4Auth(db) {
    ...
    try {
      ...
      if (role === ADVISOR_ROLE) {
        ...
        const { data: clients, error: lookupError } = await db.from("users")
          .select("id, first_name, surname, auth_user_id, client_user_id")
          .eq("role_id", CLIENT_ROLE_ID)
          .or(`auth_user_id.eq.${auth.id},client_user_id.eq.${auth.id}`);
        ...
        if (client.auth_user_id && client.client_user_id && client.auth_user_id !== client.client_user_id)
          return res.status(403).json({ error: "Your client account link needs to be corrected by your adviser." });
        req.user = { id: auth.id, role: "client", clientId: client.id, name: `${client.first_name} ${client.surname}`.trim() };
      }
      next();
    } catch {
