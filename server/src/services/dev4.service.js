...
const { CLIENT_ROLE_ID, PUSH_HOSTS } = require("../constants/dev4");
...
function createSharedService({ db, checkConsent, push, pushPublicKey = "" }) {
  async function result(query) {
    ...
    if (error) {
      if (["42P01", "42703", "PGRST202", "PGRST204", "PGRST205"].includes(error.code))
        fail(503, "Shared database setup is incomplete. Apply the Dev 4 database migration.");
      throw error;
    }
    return data;
  }
  ...
  async function clientFor(user, id) {
    ...
    const client = await result(db.from("users").select("id, first_name, surname").eq("role_id", CLIENT_ROLE_ID).eq("id", id).maybeSingle());
  ...
  const service = {
    ...
    async clients(user) {
      let query = db.from("users").select("id, first_name, surname").eq("role_id", CLIENT_ROLE_ID).order("first_name");
      if (user.role !== "adviser") query = query.eq("id", user.clientId);
      return (await result(query)).map(c => ({ id: c.id, name: `${c.first_name} ${c.surname}`.trim() }));
    },
    ...
    async sendMessage(user, input) {
      ...
      const row = await result(db.rpc("dev4_send_message", { p_client_id: input.clientId, p_sender_id: user.id, p_sender_name: user.name, p_sender_role: user.role, p_body: required(input.body, "Message", 4000) }));
    ...
    async publishEvent({ eventId, clientId, type, title, body, audience: target = "both" }) {
      ...
      await result(db.rpc("dev4_publish_event", { p_client_id: clientId, p_event_key: `${type}:${clientId}:${required(eventId, "Event ID")}`, p_title: required(title, "Title"), p_body: required(body, "Body", 1000), p_audience: audience(target) }));
    },
    ...
    async tick() { await result(db.rpc("dev4_run_reminders")); await service.flushPush(); },
    async flushPush() {
      if (!push) return;
      for (const n of await result(db.rpc("dev4_claim_push"))) {
        ...
        for (const s of subscriptions) {
          ...
          try {
            await push({ endpoint: s.endpoint, keys: s.keys }, { title: "Royal Square Financial", body: "You have a new update. Open Royal Square to view it.", tag: n.id, url: "/workspace#notifications" });
