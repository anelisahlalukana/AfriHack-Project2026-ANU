...
const { createDev4 } = require("./index");
...
test("HTTP routes require authentication, isolate clients and reject client rule writes", async (t) => {
  ...
  const dev4 = createDev4({ demo: true, store: createStore(), now });
  app.use("/api/dev4", dev4.router);
  const server = app.listen(0, "127.0.0.1");
  await new Promise((resolve) => server.once("listening", resolve));
  t.after(() => new Promise((resolve) => server.close(resolve)));
  const url = `http://127.0.0.1:${server.address().port}/api/dev4`;
...
test("normal mode never trusts demo headers", async (t) => {
  const app = express();
  app.use(express.json());
  app.use("/api/dev4", createDev4({ store: createStore() }).router);
  const server = app.listen(0, "127.0.0.1");
  await new Promise((resolve) => server.once("listening", resolve));
  t.after(() => new Promise((resolve) => server.close(resolve)));
  assert.equal(
    (
      await fetch(
        `http://127.0.0.1:${server.address().port}/api/dev4/clients`,
